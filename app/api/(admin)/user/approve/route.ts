...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (user.role?.name !== "Admin") {
      return NextResponse.json(errorResponse("User is not an admin", HttpStatus.BAD_REQUEST), {
        status: HttpStatus.BAD_REQUEST,
      });
    }
    ...
    const message = "Admin user activated successfully.";

    await sendUserActivationStausEmail(user.email, message);

    return NextResponse.json(successResponse(message, updatedUser), { status: HttpStatus.OK });
  } catch (error) {
