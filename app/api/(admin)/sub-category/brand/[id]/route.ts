...
export async function GET(req: Request, { params }: getSubCategoryParams) {
  try {
    ...
    const subCategoryWithBrands = await prisma.sub_category.findUnique({
      where: {
        id: +id,
      },
      include: {
    ...
    subCategoryWithBrands?.brands.forEach(item => {
    ...
    return NextResponse.json(successResponse(COMMON_CONSTANTS.SUCCESS, uniqueBrands), {
